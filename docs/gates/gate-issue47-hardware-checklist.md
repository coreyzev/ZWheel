# Gate: Issue #47 — Corey Hardware Validation Checklist

**Branch:** `codex/issue47-hardware-checklist`
**Base:** `main`
**Closes:** #47
**One concern:** Write a complete, operator-ready hardware validation runbook Corey can execute without reading implementation files.
...
```
docs/COREY_HARDWARE_TEST_CHECKLIST.md   ← NEW
```

Do NOT touch any `.kt`, `.gradle`, or other source files.

---

## Implementation spec

Create `docs/COREY_HARDWARE_TEST_CHECKLIST.md` with the following structure and content.
Write it as a numbered operator checklist — exact steps, expected results, pass/fail notes.
...
35. **Airplane mode:** Enable airplane mode while connected. App shows "Disconnected".
    Disable airplane mode. App does NOT auto-reconnect (by design in v1). Tap Scan to reconnect.
...
No build step needed. Just confirm the file exists and is readable markdown.

```bash
test -f docs/COREY_HARDWARE_TEST_CHECKLIST.md && echo "exists"
```

Commit message: `docs(test): write Corey hardware validation checklist (#47)`
