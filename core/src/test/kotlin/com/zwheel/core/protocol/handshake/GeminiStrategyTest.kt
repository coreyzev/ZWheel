...
class GeminiStrategyTest {
    @Test
    fun `unlock subscribes to uart notification and writes calculated response`() = runBlocking {
        val challenge = "43:52:58:7f:9e:5c:14:df:42:e2:62:82:62:62:62:62:62:77:f6:9c".strategyHexBytes()
        val io = FakeGattIo(notifications = mapOf(OwUuids.UART_READ to challenge))
...
private class FakeGattIo(
    ...
    override suspend fun read(characteristicId: GattCharacteristicId): ByteArray =
        error("read() must not be called on $characteristicId in the Gemini flow")
